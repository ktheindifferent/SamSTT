...
def test_missing_model_files():
    ...
    # Mock Path to avoid file system access
    with patch('stts.engine.Path') as mock_path_class:
        # Make all paths appear as non-existent files
        mock_path_instance = MagicMock()
        mock_path_instance.exists.return_value = False
        mock_path_instance.is_file.return_value = False
        mock_path_class.return_value = mock_path_instance
        
        # Also mock the Path constructor when called with file paths
        def path_side_effect(path_str):
            mock_instance = MagicMock()
            mock_instance.exists.return_value = False
            mock_instance.is_file.return_value = False
            mock_instance.parents = [MagicMock()]
            mock_instance.parents[0].joinpath = lambda x: mock_instance
            mock_instance.parents[1].joinpath = lambda x: mock_instance
            mock_instance.absolute.return_value = "/fake/path"
...
def test_import_errors():
    ...
    class TestEngineWithImportError(BaseSTTEngine):
        def initialize(self):
            self.initialized = True  # Don't actually import
        
        def transcribe_raw(self, audio_data, sample_rate=16000):
            return "test"
        
        def _check_availability(self):
            raise ImportError("non_existent_module not found")
...
def test_legacy_support_errors():
    ...
    # Patch all file operations to avoid file system issues
    with patch('stts.engine.Path') as mock_path_class, \
         patch('builtins.open', mock_open(read_data='{}')), \
         patch('stts.engine.os.getenv', return_value=''):
        
        # Setup mock Path to return non-existent paths
        def path_side_effect(path_str):
            mock_instance = MagicMock()
            mock_instance.exists.return_value = False
            mock_instance.is_file.return_value = False
            mock_instance.parents = [MagicMock(), MagicMock()]
            mock_instance.parents[0].joinpath = lambda x: mock_instance
            mock_instance.parents[1].joinpath = lambda x: mock_instance
            mock_instance.absolute.return_value = "/fake/path"
